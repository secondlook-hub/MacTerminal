...
- **Paste Images into CLI Tools** — Cmd+V with an image on the clipboard (a screenshot, an image copied from a browser or Preview) writes it to a PNG under the app's cache directory and pastes that file's path, so command-line tools that read image files — Claude Code among them — can pick it up straight from the prompt. Copied files paste their path too, text pastes as before, and pasted images older than a week are cleaned up automatically
