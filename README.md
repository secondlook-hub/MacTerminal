...
- **Idle Terminals Stay Idle** — The cursor lives in its own layer and blinks through Core Animation instead of a 500 ms timer that redrew the cursor cell. Any redraw makes the compositor allocate this window's render buffers, and a twice-a-second redraw never let them go: an idle, empty terminal held 338 MB. It now settles around 90 MB with no measurable CPU use. Unfocused windows show a steady cursor, as before
...
- **Snappy Typing Under Fancy Prompts** — Keystrokes no longer lag with prompt themes like Powerlevel10k or Starship. Those themes redraw the prompt on every keystroke and re-emit the same window-title/CWD escape each time; the app was forwarding every one as a change, and an unchanged title still fired an observable update that re-laid-out the entire window (sidebar, splits, tab bar) once per key. Title and directory changes are now reported only when the value actually changes, so an unchanged prompt redraw costs nothing
- **Readable Timestamps + Reliable Toggle** — The line-number and timestamp gutters now take their color from the terminal's own text color instead of the theme's status-bar color, so they stay legible under a custom background (e.g. a black background chosen under the light theme, where the timestamp used to vanish). The View > Show Timestamp checkmark now updates immediately when toggled per tab
- **Text Adapts to a Custom Background** — Picking a custom background color alone no longer leaves the body text, line numbers, and timestamps unreadable. When you haven't set your own text color, the default text and the gutters now follow the background's brightness — dark text on a light background, light text on a dark one — so a black (or bright cyan) background stays readable instead of showing near-invisible text. Setting an explicit text color still overrides this and is respected as-is
- **Line Numbers & Timestamps Actually Take Their Color** — The gutter now paints in its computed color instead of always black-on-black. Its cached Core Text lines carried no foreground attribute, so `CTLineDraw` ignored the fill color set for them and fell back to black — invisible on a dark background no matter how the gutter color was computed. The lines now opt into the context's fill color, so the readable color derived from the background finally reaches the screen
